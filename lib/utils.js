...
const index_js_2 = __importDefault(require("./entry/index.js"));
...
function isBatchOverrides(arg) {
    return compareSets(new Set(arg), overrides_js_1.highLevelHeaderOverrideSet) || compareSets(new Set(arg), overrides_js_1.highLevelControlOverrideSet);
}
...
function overrideLowLevel(values, options, self) {
    ...
    if (isEntryOverrides(values) && isEntryOptions(options) && self instanceof index_js_2.default) {
        // For each override value, check to see if it exists on the options object & if so, set it
        values.forEach((field) => {
            if (options[field])
                self.set(field, options[field]);
