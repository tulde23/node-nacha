type CamelToTitleCase<Text extends string, $Acc extends string = ''> = Text extends `${infer $Ch}${infer $Rest}` ? Text extends Capitalize<Text> ? CamelToTitleCase<$Rest, `${Capitalize<$Acc>} ${Capitalize<$Ch>}`> : CamelToTitleCase<$Rest, `${$Acc}${$Ch}`> : Capitalize<$Acc>;
...
export type BatchHeaderField<Key extends BatchHeaderKeys = BatchHeaderKeys> = {
    name: CamelToTitleCase<Key>;
    value: Key extends 'serviceClassCode' ? `${number}` : number | string;
} & BaseFieldParams;
export type BatchHeaderFieldWithBlank<Key extends BatchHeaderKeys = BatchHeaderKeys> = BatchHeaderField<Key> & {
    blank: boolean;
};
...
export type BatchControlField<Key extends BatchControlKeys = BatchControlKeys> = {
    name: CamelToTitleCase<Key>;
    number?: boolean;
    value: number | string;
} & BaseFieldParams;
export type BatchControlFieldWithNoValue<Key extends BatchControlKeys = BatchControlKeys> = {
    name: CamelToTitleCase<Key>;
    number?: boolean;
    value?: number | string;
} & BaseFieldParams;
export type BatchControlFieldWithBlank<Key extends BatchControlKeys = BatchControlKeys> = BatchControlField<Key> & {
    blank: boolean;
};
...
export type EntryFieldWithNumberStringValue<Key extends EntryFieldKeys = EntryFieldKeys> = EntryField<Key> & {
    value: `${number}`;
} & BaseFieldParams;
export type EntryFieldWithOptionalValue<Key extends EntryFieldKeys = EntryFieldKeys> = {
    name: CamelToTitleCase<Key>;
    number?: boolean;
    value?: `${number}`;
} & BaseFieldParams;
